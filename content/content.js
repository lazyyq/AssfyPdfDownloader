...
console.log('%c Chrome Extension: Content script loaded ', 'background: #222; color: #bada55');
...
function addDownloadButton() {
  ...
  const btnGroups = document.querySelectorAll('nav.navbar-static-top > div.container-fluid > div.btn-toolbar > div.btn-group');
...
async function handleDownload() {
  ...
  try {
    ...
    const originalPage = currentPageLabel ?
      parseInt(currentPageLabel.textContent.split('/')[0].trim()) : 1;

    // Go to first page
    const firstPageBtn = document.querySelector('div.btn-toolbar > div.btn-group > button[command=firstPage]');
    firstPageBtn.click();
    ...
    while (true) {
      ...
      const nextButton = document.querySelector('div.btn-toolbar > div.btn-group > button[command=nextPage]');
...
function waitForImage(pageNum) {
  return new Promise((resolve) => {
    const checkImage = () => {
      const container = document.querySelector(`div.book-container > div.book > div.page-wrapper[page="${pageNum}"]`);
      if (!container) {
        resolve(null);
        return;
      }
